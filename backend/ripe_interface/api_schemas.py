...
class AnomalyOut(Schema):
    ...
    timestamp: str = Field("2022-4-13 8:41:28",
                           description="Date and time when the anomaly occurred. Format: yyyy-mm-dd "
                                       "hh:mm:ss")
    ip_address: str = Field("localhost", description="Target address of Autonomous System")
    ...
    medium_value: float = Field(40.122, description="Average increase of round trip time (ms) at time of anomaly.")
    value: float = Field(12.144, description="Average RTT (ms) at time of anomaly")
    ...
    prediction_value: bool = Field(True,
                                   description="User feedback, whether the prediction was accurate (True) or not. (False)")
    asn_error: int = Field(1402, description="ASN of the router were the error happened.")
